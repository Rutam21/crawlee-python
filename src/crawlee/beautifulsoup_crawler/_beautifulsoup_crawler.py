...
class BeautifulSoupCrawler(BasicCrawler[BeautifulSoupCrawlingContext]):
    ...
    async def _make_http_request(self, context: BasicCrawlingContext) -> AsyncGenerator[HttpCrawlingContext, None]:
        ...
        Yields:
            The enriched crawling context with the HTTP response.
        """
    ...
    async def _handle_blocked_request(
        ...
        """Handles blocked requests by checking the HTTP status code.

        Args:
            context: The current crawling context.

        Raises:
            SessionError: If the session is blocked based on the HTTP status code.

        Yields:
            The same context if no errors are detected.
        """
    ...
    async def _parse_http_response(
        ...
        """Parses the HTTP response using the `BeautifulSoup` library and implements the `enqueue_links` function.

        Args:
            context: The current crawling context.

        Yields:
            The enriched crawling context with the `BeautifulSoup` selector and the `enqueue_links` function.
        """
