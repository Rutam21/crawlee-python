...
class ParselCrawler(BasicCrawler[ParselCrawlingContext]):
    ...
    async def _parse_http_response(
        ...
        """Parses the HTTP response using the `Parsel` library and implements the `enqueue_links` function.

        Args:
            context: The current crawling context.

        Yields:
            The enriched crawling context with the `Parsel` selector and the `enqueue_links` function.
        """
